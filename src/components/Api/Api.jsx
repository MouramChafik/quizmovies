...
function Api({
  ...
  useEffect(() => {
    ...
    const { VITE_MY_API_KEY } = import.meta.env;
    (async () => {
      const movie = await fetch(
        `https://api.themoviedb.org/3/discover/movie?include_adult=false&include_video=true&api_key=${VITE_MY_API_KEY}&language=fr-FR&page=${aleaPage}&sort_by=revenue.desc&with_original_language=en|fr`
      );
